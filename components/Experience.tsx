  ...
  return (
    <SectionReveal className="px-6 py-24 md:px-8 md:py-28">
      <div id="experience" className="mx-auto w-full max-w-6xl scroll-mt-24">
        <p className="text-xs tracking-[0.12em] text-accent">$ cat ./experience.log</p>
        ...
        <div className="relative mt-12 space-y-10 pl-8 md:pl-12">
          <span className="absolute left-1.5 top-2 h-[calc(100%-16px)] w-px bg-border md:left-2" />

          {experience.map((item) => (
            <article key={`${item.role}-${item.period}`} className="relative">
              <span className="absolute -left-[1.92rem] top-1 h-3.5 w-3.5 border border-accent/80 bg-accent/70 md:-left-[2.45rem]" />
